...
-- Seed idempotente del catÃ¡logo del esquema archivos.
-- Estrategia: MERGE (MATCHED + NOT MATCHED BY TARGET) + IDENTITY_INSERT.
-- No incluye DELETE de huÃ©rfanos.
--
-- cat_entidad_vinculable: entidades de negocio a las que se puede asociar un
-- archivo por el vÃ­nculo polimÃ³rfico. id explÃ­cito = contrato estable entre
-- ambientes (dev, QA, prod). columna_pk sigue la convenciÃ³n id_<entidad>.
--
-- Se excluyen a propÃ³sito:
-- * CatÃ¡logos, tablas puente y de detalle (no reciben adjuntos).
-- * Entidades que ya manejan su archivo canÃ³nico por FK fÃ­sica
...
-- esquema, su entidad (tabla) y su columna PK. No requiere migraciÃ³n versionada.
...
-- (pueden quedar huecos en la numeraciÃ³n; es correcto).
-- =============================================================================

-- -----------------------------------------------------------------------------
-- cat_tipo_archivo: tipos de archivo. Requerido por archivos.archivo
-- (id_tipo_archivo es NOT NULL con FK a este catÃ¡logo): sin estos registros no
-- es posible cargar archivos. Conjunto inicial; ajustar codigos/limites segÃºn
-- necesidad. es_multimedia: 1 = audio/video/imagen, 0 = no. tamanio_max_mb NULL
-- = sin lÃ­mite especÃ­fico del tipo.
-- -----------------------------------------------------------------------------
...
USING (VALUES
 ...
 (7, N'PLANILLA', N'Planilla de cÃ¡lculo', 0, 50),
 (8, N'PRESENTACION',N'PresentaciÃ³n', 0, 100),
 (9, N'CORREO', N'Correo electrÃ³nico', 0, 50),
...
WHEN NOT MATCHED THEN INSERT
 ...
 VALUES
 (src.id_tipo_archivo, src.codigo, src.nombre, src.es_multimedia, src.tamanio_max_mb, CURRENT_TIMESTAMP);
...
WHEN NOT MATCHED THEN INSERT
 ...
 VALUES
 (src.id_entidad_vinculable, src.esquema, src.entidad, src.columna_pk, 1, CURRENT_TIMESTAMP);
