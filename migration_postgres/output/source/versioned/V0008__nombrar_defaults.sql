-- =============================================================================
-- SIP — Migration V0008__nombrar_defaults
-- =============================================================================
-- Versión:      4.0 v7 (incremental sobre V0007)
...
--   (con prefijo DF__ y sufijo hexadecimal aleatorio) al patrón
--   institucional df_<tabla>_<columna>.
--
-- JUSTIFICACIÓN:
--   En V0001 (y subsecuentes hasta V0006), las columnas con cláusula DEFAULT
--   inline se declararon sin nombre explícito de constraint. SQL Server les
--   asignó nombres autogenerados (ej: DF__denuncia__estado__6C6E1476) cuyo
--   sufijo hexadecimal varía entre ambientes.
--
--   Esto generó un problema operativo en V0007 al intentar dropear las 3
--   columnas de estado de denuncias.denuncia: el motor exige eliminar primero
--   los DEFAULT constraints dependientes, lo que requirió un bloque dinámico
--   de descubrimiento (sys.default_constraints + sys.columns) para resolverlos.
--
--   El mismo problema se manifestará en cualquier migration futura que toque
--   columnas con DEFAULT autogenerado. La solución estructural es renombrar
--   los 279 constraints autogenerados al patrón institucional, dejando el
--   modelo consistente con la convención de naming declarada en §3.3 del
--   Diseño del Modelo de Datos: pk_<tabla>, fk_<tabla>_<atributo>,
--   ix_<tabla>_<columnas>, uq_<tabla>_<columnas>, ck_<tabla>_<columna>,
--   df_<tabla>_<columna>.
--
-- IDEMPOTENCIA:
--   El script descubre dinámicamente los DEFAULT constraints cuyo nombre
--   sigue el patrón autogenerado (DF__*) y los renombra al patrón
...
--
-- IMPACTO:
--   sp_rename sobre constraints es atómico, no reescribe tablas, no toca
--   datos, no bloquea operaciones. Tiempo estimado de ejecución: < 10 segundos
--   sobre la línea base 4.0.
-- =============================================================================
SET ANSI_NULLS ON;
GO
...
-- =============================================================================
-- Script dinámico de renombrado
-- =============================================================================
SET NOCOUNT ON;
GO

DECLARE @rename_sql NVARCHAR(MAX) = N'';
...
SELECT
 ...
 WHERE s.name IN (
   ...
   -- Solo renombrar autogenerados (patrón DF__<tabla>__<col>__<hex>)
...
-- distintas tablas que generen el mismo df_<tabla>_<columna>; no debería
...
-- Verificar también que el nombre nuevo no exista ya como otro objeto
...
BEGIN
    PRINT N'V0008: ATENCIÓN - hay ' + CAST(@colisiones_existentes AS NVARCHAR(10))
...
PRINT N'V0008: ' + CAST(@total_a_renombrar AS NVARCHAR(10))
...
BEGIN
    BEGIN TRY
        DECLARE @full_old_name NVARCHAR(300) = QUOTENAME(@s) + N'.' + QUOTENAME(@old);
...
PRINT N'V0008: Renombrados exitosos: ' + CAST(@count_renombrados AS NVARCHAR(10));
PRINT N'V0008: Omitidos por error:    ' + CAST(@count_omitidos AS NVARCHAR(10));
GO


-- =============================================================================
-- FIN — V0008__nombrar_defaults
-- =============================================================================
-- Validación recomendada (después de ejecutar):
--
--   -- Listar DEFAULTs que aún tengan nombre autogenerado (debería ser 0
--   -- después de aplicar V0008, salvo que se hayan creado nuevos en
...
--   -- esperado: 0 filas
--
--   -- Conteo de DEFAULTs que siguen el patrón df_<tabla>_<columna>
--   SELECT COUNT(*)
--     FROM sys.default_constraints dc
--     JOIN sys.tables  t ON t.object_id = dc.parent_object_id
--     JOIN sys.schemas s ON s.schema_id = t.schema_id
